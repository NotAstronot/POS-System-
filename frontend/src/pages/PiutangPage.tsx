...
import { adminService, Receivable } from '../services/api';
...
export default function PiutangPage() {
  ...
  const [form, setForm] = useState({ debtor_name: '', amount: '', description: '', due_date: '', paid_amount: '0' });

  const load = async () => {
    try { const res = await adminService.listReceivables(); setData(res.data.data); } catch {} finally { setLoading(false); }
  };
  useEffect(() => { load(); }, []);

  const openAdd = () => { setEditItem(null); setForm({ debtor_name: '', amount: '', description: '', due_date: '', paid_amount: '0' }); setShowForm(true); };
  const openEdit = (d: Receivable) => { setEditItem(d); setForm({ debtor_name: d.debtor_name, amount: String(d.amount), description: d.description, due_date: d.due_date || '', paid_amount: String(d.paid_amount) }); setShowForm(true); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editItem) {
        ...
        await adminService.updateReceivable(editItem.id, payload);
      } else {
        delete payload.paid_amount;
        await adminService.createReceivable(payload);
      }
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await adminService.deleteReceivable(id); load(); } catch {}
  };
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={openAdd} className="bg-blue-600 text-white px-4 py-2 rounded">+ Piutang</button>
      </div>

      {showForm && (
        <div className="bg-white p-4 rounded shadow mb-6">
          <h2 className="font-bold mb-4">{editItem ? 'Edit Piutang' : 'Tambah Piutang'}</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
            <input placeholder="Nama Debitur" value={form.debtor_name} onChange={e => setForm({ ...form, debtor_name: e.target.value })} className="border p-2 rounded" required />
        ...
        <div className="bg-white rounded shadow overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-100">
              <tr>
                <th className="p-3 text-left">Debitur</th>
            ...
            <tbody>
              {data?.map(d => (
                <tr key={d.id} className="border-t">
                  <td className="p-3 font-medium">{d.debtor_name}</td>
              ...
              {data?.length === 0 && <tr><td colSpan={7} className="p-4 text-center text-gray-400">Belum ada data piutang</td></tr>}
            </tbody>
          </table>
        </div>
      )}
